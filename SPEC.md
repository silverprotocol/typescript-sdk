...
44. **MCP resource links (draft.5)** — binds the normalizers that §8.0 item 31 binds. Feed such a normalizer a tool result whose `content[]` holds N MCP `resource_link` parts. It emits exactly N `resource-link` blocks in `tool.done.content`, in native order. Each block's `uri`, `name`, `title`, `description`, `mimeType`, `size`, `annotations` and `_meta` deep-equal the native members where present and are absent where the native member is absent. No `provider-raw` block in that `tool.done` carries a member `type` equal to `"resource_link"`. A native part carrying `icons`, or a `size` that is not a number, is followed by exactly one `provider-raw` block carrying exactly those members; a native part carrying neither is followed by none. Framework-specific legs follow §8's applicability scoping.
