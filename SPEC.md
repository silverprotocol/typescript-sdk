...
>
> **draft.5 (2026-09-25)** changes how `reduce()` folds a `messages.snapshot` (§5.0 INV-MSG, §5, §10 item 27): a turn is closed while its turn record carries an `outcome`, so a snapshot no longer clears closure — one that carries a turn with an `outcome` keeps it closed, and one that omits `turns` keeps every closure. A `message.start`, or a `tool.done` adopting a message, into such a turn after a snapshot now parks the reducer where draft.4 folded it; a host that means to reopen a turn carries its record without an `outcome`. A turn record that carries an `outcome` without having been closed, as a reducer's record for an ambiguous `turnId`-less terminal can, is now closed. This change alters no producer's bytes.
>
...
A conformant normalizer ships fixtures asserting the items below. Items that name a reducer, a consuming ingest or a stored-record reader (1, 2, 20, 22, 29, 37, 39) bind those implementations instead, as do the first sentences of items 27 and 43 and item 35, which bind reducers. A replay golden is every golden committed to the reference corpus, including a committed capture not yet enrolled in replay. The input leg of item 29 binds implementations that ship an input receiver, and ingest-only normalizers record it N/A.
