...
- A receiver of an input passes unknown object fields through untouched, as above. An input can carry, at any depth, a value this specification does not define in a closed set, for example in `AgInput.kind`, in `AgHitlAnswer.status`, in `AgReasoningConfig.effort`, or as an `AgBlock` `type` inside `messages`, `run.system`, `run.context` or `results[].content`. The receiver MUST then reject the whole input before acting on any part of it. The rejection is a typed error that names the value's path and classifies it as `unknown-value` (a string the set does not define), which is distinct from `malformed` (a missing value or a value of the wrong JSON type). A sender that receives `unknown-value` at a path inside an `AgBlock[]` MAY resend the input with that element removed.
