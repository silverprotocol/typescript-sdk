...
> **Status:** Draft (2026-09-23) · **Spec version: `1.0.0-draft.4`** (the value `AgInputEnvelope.version` carries; negotiation rules in §12) · **License:** MIT (see `LICENSE`; applies to this specification text and the reference SDK). The open, neutral, typed format for **agent ↔ user-client** communication — input, output, and UI-interaction anchors. Home: `silverprotocol.io/AgJSON`. Normalizers (framework I/O ⇄ AgJSON) ship as `@silverprotocol/<framework>`.
>
...
**INV-MSG (message closure & seal).** Every `message.start` is paired with exactly one `message.end`; `flush()` synthesizes `message.end` for each still-open message in insertion order. `message.end` **seals** the message: a block-creating event, a delta event, or a block-finalizing event (exactly `text.end`, `reasoning.end`, `reasoning.opaque`, `tool.args.assembled`) targeting a sealed message — or any message of a closed turn — is a `reduce()`-error → snapshot-resync, never a silent attach. A `message.start` naming a closed turn is likewise a `reduce()`-error → snapshot-resync; it never opens a message under that turn. A turn is closed once its `turn.done`, `turn.error` or `turn.abort` has folded; a later `turn.start` or `subagent.start` naming it merges fields (INV-TURN) and does not reopen it. Closure lasts for the rest of the fold: unlike INV-BLOCK's id scope it survives a 0-restart, and only a `messages.snapshot` clears it, with the rest of the transient `reduce()` state (§5 table). This rule covers block events and `message.start` only: the message-level merges `message.metadata` and `turn.done.messageMetadata` fold onto a named message as their §5 rows state. `message.remove` of a sealed message un-seals it per the pointer-revert rule (§5 table).
...
| `turn.done`           | Close the turn; record `finishReason`/`finishReasonRaw`/`usage`/`safety`/`taskState`; if `outcome.type==="paused"`, record the asks; `messageMetadata` REPLACE-merges onto the message named by `turn.done.messageId` when present, else the open message of the turn. `finishReason:"pause_turn"` records a resumable checkpoint with **no** asks[] (re-input is a plain replay, NOT a resume kind). |
...
**Version negotiation.** The current spec version is `1.0.0-draft.4`. Emitters MUST populate `AgInputEnvelope.version` (and SHOULD surface the same value out-of-band) with the spec version they implement. A consumer MUST reject an envelope whose `version` differs in MAJOR component, and SHOULD accept any same-major version, ignoring unknown additive fields per §0.2's consumer-ingestion posture. Behavior on prerelease tags (`-draft.*`) is same-major acceptance.
