...
> **Status:** Draft (2026-08-15) · **Spec version: `1.0.0-draft.2`** (the value `AgInputEnvelope.version` carries; negotiation rules in §12) · **License:** MIT (see `LICENSE`; applies to this specification text and the reference SDK). The open, neutral, typed format for **agent ↔ user-client** communication — input, output, and UI-interaction anchors. Home: `silverprotocol.io/AgJSON`. Normalizers (framework I/O ⇄ AgJSON) ship as `@silverprotocol/<framework>`.
>
> **draft.2 (2026-08-15)** adds two constructs, both additive (an absent field/role is byte-identical to draft.1): **grant modes** on the HITL ask (`grantModes` on `hitl.ask`/`AgPausedAsk`, `grantModeId` on `AgHitlAnswer`, the `hitl.grantModes` client capability — §7), letting an asker declare scope-qualified accept variants (anchored on the Claude Agent SDK's `permission_suggestions`/`PermissionUpdate.destination` surface and the OpenAI Agents `approve(…, {alwaysApprove})` option; spec-process record: typescript-sdk#16); and the **`notice` role** (§3, §3.2) — a persisted, non-conversational transcript row with `noticeSource` provenance, promoting the Claude informational carry (§10 item 21) and giving host/adapter-injected annotations a first-class home.
>
...
- **`noticeSource?: "host" | "adapter" | "framework"`** (on `AgMessage` / `message.start`, meaningful only when `role:"notice"`) names the architecture layer that injected the row: `host` = the application; `adapter` = the normalizer/bridge layer synthesized it; `framework` = the normalized framework stream emitted it (e.g. the Claude informational banner, §10 item 21).
