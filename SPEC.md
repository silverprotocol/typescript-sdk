...
30. **ADK shared state (draft.4)** — an ADK normalizer emits each `actions.stateDelta` it receives as one `state.delta` whose `patch` is a key-replace object (§5) holding the map's top-level entries with their values unchanged, less every `temp:` entry and every entry the credential rule omits; it adds no nesting and emits no JSON Patch. The resulting fold is the per-key replace of the deltas ADK yields; it is not ADK's session state, which also holds host-seeded values and writes ADK applies without yielding them.
