...
`turn` (the exchange) · `step` (one loop iteration) · `block` (a content unit) · `tool-call` / `tool-result` · `reasoning` (never "thinking") · `hitl` (the human-in-the-loop family) · `ask`/`answer` (the HITL request/response — `ask` materializes as the `hitl.ask` event, `answer` materializes as the `AgHitlAnswer` resume payload; there is **no** `hitl.answer` wire type) · `opaque` (provider-bound verifiable blob) · `paused` (a within-run wait — the word `interrupt` is purged everywhere, including async-tool scheduling, which uses `preempt`) · `surface` (an A2UI / MCP-Apps / OpenAI-Apps live render target) · `surface-interaction` (a client→server surface message — `AgSurfaceInteraction`, the un-merged successor of the old `AgUiAction`, a DIFFERENT axis from the `hitl` pause) · `notice` (a persisted, non-conversational transcript row — never "system message", which names the instruction role) · `grant mode` (a declared accept variant on a HITL ask — never "permission" or "scope" alone, which name host policy, not wire vocabulary). Type names are `Ag<Noun>` PascalCase.
