...
27. **Re-delivery never folds twice (draft.4)** — within one invoke, a re-delivered event (`0 < seq <= lastSeq`), a second block-creating `*.start` for an id already present, a delta or block-creating event into a sealed message or into any message of a closed turn, and a second `more`-less `tool.done` for one `toolCallId` each leave `needsResync` true and the fold equal to the fold of the events before it; a later invoke restarting at `seq` 0 that reuses a block id folds normally. On every replay golden, block-creating `*.start` ids are unique within each invoke.
