...
>
> **draft.5 (2026-09-25)** changes how `reduce()` folds a `messages.snapshot` (§5.0 INV-MSG, §5, §10 item 27): a turn is closed while its turn record carries an `outcome`, so a snapshot no longer clears closure — one that carries a turn with an `outcome` keeps it closed, and one that omits `turns` keeps every closure. A `message.start`, or a `tool.done` adopting a message, into such a turn after a snapshot now parks the reducer where draft.4 folded it; a host that means to reopen a turn carries its record without an `outcome`. A turn record that carries an `outcome` without having been closed, as a reducer's record for an ambiguous `turnId`-less terminal can, is now closed. This change alters no producer's bytes. draft.5 also widens the `resource-link` block with MCP's optional `name`, `title`, `description` and `size` (additive, §12); adds §8.0 item 31 (MCP resource links in tool results) and §10 item 44; adds §2.1's host-record note, and extends §13.4's consumer scheme validation to `resource-link` uris and to links a consumer takes from a tool result; and moves §8.0 item 21's notice siblings and item 19's audit copy from `providerMetadata` to host-only `_meta` (§10 item 45). Output is NOT byte-identical to draft.4 for google-adk (MCP resource links in `tool.done.content`) or for claude-agent-sdk: host-only wrapper keys move to `_meta`, and on a frame whose first block is a tool call they arrive on a `message.metadata` event.
>
...
type AgInput =
  ...
  // Resume: answer a paused turn's asks and/or carry surface interactions (§3.1); the resuming invoke's events open a new turn.
  | (AgInputEnvelope & {
      kind: "resume";
...
> **`outputTokens` inclusion (draft.3).** `outputTokens` counts every token the provider generated for the turn or message and bills at its output rate — visible text AND reasoning/thinking. `reasoningTokens`, when present, is a BREAKDOWN of `outputTokens`, never an additional bucket: consumers MUST NOT add the two. A normalizer whose provider reports reasoning as a SIBLING counter (Gemini `usageMetadata.thoughtsTokenCount` beside `candidatesTokenCount`) MUST fold it in — `outputTokens = candidatesTokenCount + thoughtsTokenCount` — unless the provider's own total shows the sibling is already included (`promptTokenCount + candidatesTokenCount + toolUsePromptTokenCount == totalTokenCount` with thoughts present), in which case `candidatesTokenCount` is carried as-is. This fold is the one sanctioned arithmetic on usage: INV-DELTA's verbatim rule (§5.0) governs cumulative streams and §8.0 item 19's no-subtraction posture stands. `totalTokens` is copied from the provider or framework wire, never computed by a normalizer, and MAY be absent (Anthropic reports none); where it is present a conformant fold satisfies `inputTokens + outputTokens + (toolUseInputTokens ?? 0) == totalTokens` (§10 item 21). `outputTokens − reasoningTokens` approximates visible-text output and is a consumer derivation — approximate on providers that estimate reasoning by re-tokenization (Anthropic `thinking_tokens`); `reasoningTokens` SHOULD be ≤ `outputTokens` when both come from the same provider snapshot. **Scope:** this note defines the OUTPUT side only. `inputTokens`' relation to `cacheReadTokens`/`cacheWriteTokens` remains provider-shaped (Anthropic `input_tokens` EXCLUDES cache tokens; OpenAI, Gemini and Vercel INCLUDE them) pending a separate revision.

An `AgUsage` key this specification does not define is an annotation: consumers MUST pass it through (§0.2) and MUST NOT add it into any total or into the §10 item 21 identity.
...
| `turn.error`          | Terminal; mark the turn errored: fold `outcome = {type:"error", message, code?}` on the turn record (INV-MSG). **Non-folding into content** (no block produced). Record `usage` verbatim when present (mirrors `turn.done`; the accrued cost of the errored turn).                          |
...
14. **OpenAI deferred round-close** — the OpenAI Agents SDK wire delivers a round's `tool_output` run-item AFTER `response.completed`; a normalizer MUST defer that round's `message.end` + `turn.done` until the round's pending tool results have landed (or stream end, releasing the stashed close at flush per INV-FLUSH (2), never as a success `turn.done`) so a tool result never targets a sealed message or closed turn (INV-MSG, §5.0). A host-fed error (host-binding obligation 1) that arrives while a round's close is deferred releases that close per INV-FLUSH (2) before the normalizer closes the error.
...
A conformant normalizer ships fixtures asserting the items below. Items that name a reducer, a consuming ingest or a stored-record reader (1, 2, 20, 22, 29, 37, 39) bind those implementations instead, as do the first sentences of items 27 and 43, item 35 and the last sentence of item 45, which bind reducers. A replay golden is every golden committed to the reference corpus, including a committed capture not yet enrolled in replay. The input leg of item 29 binds implementations that ship an input receiver, and ingest-only normalizers record it N/A.
...
27. **Re-delivery never folds twice (draft.4)** — within one invoke, a re-delivered event (`0 < seq <= lastSeq`), a second block-creating `*.start` for an id already present, a delta or block-creating event into a sealed message or into any message of a closed turn, a `message.start` naming a closed turn (INV-MSG), and a second `more`-less `tool.done` for one `toolCallId` each leave `needsResync` true and the fold equal to the fold of the events before it; a later invoke restarting at `seq` 0 that reuses a block id folds normally, while a later invoke's `message.start` naming a turn that an earlier invoke of the fold closed leaves `needsResync` true and the fold equal to the fold of the events before it; and a re-sent `turn.start` plus a second `turn.done{paused}` for a turn already closed paused, with refreshed `asks[]`, does not park: the fold keeps one record for the turn, and that record carries the later `asks`. Across a `messages.snapshot`, closure follows the fold's turn records (INV-MSG): after a snapshot that carries turn X with an `outcome`, a `message.start` naming X and a `tool.done` whose `messageId` adopts a message of X each leave `needsResync` true and the fold equal to the fold of the events before it, whether the reducer closed X before the snapshot or joined at it; a snapshot that omits `turns` after X closed keeps X closed; a snapshot that carries X without an `outcome`, or carries `turns: []`, leaves X open, and the same `message.start` folds; and a completed invoke re-sent from `seq` 0 after a snapshot of its own fold leaves `needsResync` true and the fold equal to the snapshot's. A second terminal of a different type (a `success` or error after `paused`, or any terminal after `turn.error` or `turn.abort`) is not covered by this item. The reference suite checks the §8.0 producer duties on its corpus: on every replay golden, block-creating `*.start` ids are unique within each invoke and no `message.start` follows the terminal of the turn it names; on every committed resume golden folded after the golden of the invoke it resumes, no turn id or message id of one invoke recurs in the other.
