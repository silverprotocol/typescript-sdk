...
A conformant normalizer ships fixtures asserting the items below. Items that name a reducer, a consuming ingest or a stored-record reader (1, 2, 20, 22, 29, 37) bind those implementations instead, as do the first sentence of item 27 and item 35, which bind reducers. A replay golden is every golden committed to the reference corpus, including a committed capture not yet enrolled in replay. The input leg of item 29 binds implementations that ship an input receiver, and ingest-only normalizers record it N/A.
