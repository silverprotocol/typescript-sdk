...
30. **ADK shared state (draft.4)** — an ADK normalizer emits each `actions.stateDelta` it receives as one `state.delta` whose `patch` is a key-replace object (§5) holding the map's top-level entries with their values unchanged, less every `temp:` entry and every entry item 28 omits; it adds no nesting and emits no JSON Patch. The resulting fold is the per-key replace of the deltas ADK yields; it is not ADK's session state, which also holds host-seeded values and writes ADK applies without yielding them.
...
31. **Credential objects omitted from shared state (draft.4)** — for a normalizer targeting ADK, synthetic native events whose `actions.stateDelta` carries framework credential objects (as an entry's value and nested within it, in both spellings, including an ADK auth configuration stored whole under one key, whose credential objects sit one level down; a credential whose only member besides `authType` is `resourceRef`; and a `serviceAccount` credential whose `serviceAccount` member is `{useDefaultCredential: true}`), `temp:`-prefixed entries, an own `__proto__` key and ordinary entries yield, per native event, exactly one `state.delta` whose `patch` deep-equals the ordinary entries alone (`{}` when none remain), and the same sequence of event types as the same events with those entries removed. No emitted event and no part of `reduce(events).result` contains a value from an omitted entry, and no omitted key appears in `reduce(events).result.state` or as an own key of any emitted `patch`. An ordinary entry, including an object whose `authType` is not one of ADK's credential types, that has no credential member, or whose `resourceRef` is `null`, is carried byte-identical. `push()` does not throw on a state map that is not an object.
